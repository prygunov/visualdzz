...
public class BlurForm extends JFrame{
    ...
    List<Tick> generateTicks(){
        ...
        //генерация одинаковых тактов с разными названиями
        for(int i = 0; i< n;i++) {
    ...
    public void updateChart(JPanel rootPanel, List<Tick> ticks){
        ...
        double offset = Math.abs((float) b / n); //сдвиг тактов относительно друг друга
        ...
        for(int i = 0; i< ticks.size();i++) {
            ...
            var beginFrom = offset * i; //сдвиг каждого такта
        ...
        for (double x = min; x <= max; x+=step) {
            ...
            for(XYSeries series : (List<XYSeries>)dataset.getSeries()){
                ...
                //подсчет времени экспонирования
            }
